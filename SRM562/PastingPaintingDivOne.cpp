...
public:
	...
	vector<long long> countColors(vector <string> clipboard, int T) {
		...
		m = clipboard[0].size();
		REP(i, n) REP(j, m) a[i][j] = getc(clipboard[i - 1][j - 1]);
		// calculate the first T - n + 1
		//if (T - n + 1 > 0){
		int N = min(n, m);
			REP(i, N){
				rep(j, i, m){
					if (a[i][j] > 0){
						int cks = 1;
						REP(k, i - 1)
							if (a[i - k][j - k] > 0) cks = 0;
						if (cks){
							ans[a[i][j]] += max(T - N + 1, 0);
						}
					}
				}
				rep(j, i, n){
					if (i != j && a[j][i] > 0){
						int cks = 1;
						REP(k, i - 1)
							if (a[j - k][i - k] > 0) cks = 0;
						if (cks){
							ans[a[j][i]] += max(T - N + 1, 0);
						}
					}
				}
			}
