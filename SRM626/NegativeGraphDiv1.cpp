...
const int MAX_N = 55, MAX_M = 2555;

class NegativeGraphDiv1 {
public:
	int g[MAX_M], nxt[MAX_M], c[MAX_N], cost[MAX_M], nm;
	queue<int> q;
	LL d2[MAX_N], d[MAX_N];
	int iq[MAX_N], M;
	int n;
	LL ans[MAX_N][MAX_N], a[MAX_N][MAX_N], b[MAX_N][MAX_N];
	
	void addedge(int x, int y, int w){
		g[nm] = y; nxt[nm] = c[x]; c[x] = nm; cost[nm] = w; nm++;
	}
	...
	void spfa(){
		while (!q.empty()){
			...
			for (int k = c[i]; ~k; k = nxt[k]){
				...
				if (d[j] > d[i] + cost[k]){
					d[j] = d[i] + cost[k];
					if (!iq[j]) q.push(j), iq[j] = 1;
				}
			}
		}
	}
	
	void mul(LL c[][55], LL a[][55], LL b[][55]){
		REP(i, n) REP(j, n) c[i][j] = 1000000000000000000LL;
	...
	long long findMin(int N, vector <int> from, vector <int> to, vector <int> weight, int charges) {
		...
		memset(c, -1, sizeof c);
		n = N;
		REP(i, M) addedge(from[i - 1], to[i - 1], weight[i - 1]);
		
		REP(i, n){
			REP(j, n) d[j] = d2[j] = 1000000000000000000LL;
			d[i] = 0;
			q.push(i), iq[i] = 1;
			spfa();
			REP(j, n) ans[i][j] = d[j];
			REP(j, n){
				...
				for (int k = c[j]; ~k; k = nxt[k])
					d2[g[k]] = min(d2[g[k]], d[j] - cost[k]);
			}
			REP(j, n) d[j] = d2[j];
			REP(j, n) q.push(j), iq[j] = 1;
			spfa();
			REP(j, n) a[i][j] = d[j];
		}
		//REP(i, n) REP(j, n) printf("a[%d,%d] =  %lld\n", i, j, a[i][j]);
		bool t = 0;
		for (int z = 0; z <= 30; ++z){
			if ((charges >> z) & 1){
				//if (t == 0)
